...
//		bytesPerPixel = infoHeader.bBitCount / 8;
	...
	compressed_matrix = matrixmultiplic(pixel, width, simplingmatrix, 190);
	...
	compressed_matrix = matrixmultiplic(wwturn, width, compressed_matrix,
			190);
	simplingmatrix = matrixmultiplic(wwturn, width, simplingmatrix, 190);
	...
	//小波反变换
	temp1=matrixmultiplic(matrixturn(ans,height,width),width,ww,height);
	ans=matrixmultiplic(ans,width,temp1,height);
	//输出结果
